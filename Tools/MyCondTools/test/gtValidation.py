...
if __name__     ==  "__main__":
    ...
    if releasearea == '':
        ...
        if os.path.exists(options.release):
            ...
            scramproj = 'scram project CMSSW ' + options.release
    ...
    evaloutands = commands.getstatusoutput('eval `scramv1 runtime -sh`')
    if evaloutands[0] != 0:
        print evaloutands[1]
...
#                 print 'copy file: ' + newsqlite + ' to ' + GTSQLITESTORE
    ...
    if not os.path.exists('Tools/MyCondTools/'):
        ...
        outandstat = commands.getstatusoutput('cvs co -d Tools/MyCondTools -r VTOOLS01 UserCode/cerminar/Tools/MyCondTools')
        if outandstat[0] != 0:
            print outandstat[1]
    ...
    for gt in args:
        ...
        loadallscript_local.write("eval `scram runtime -csh`\n")
        ...
        loadallscript_frontier.write("eval `scram runtime -csh`\n")
    ...
    if not options.file == 'None':
        ...
        outandstat = commands.getstatusoutput('eval `scramv1 runtime -sh`; addpkg -f ' + options.file + " ; checkdeps -a")
        if outandstat[0] != 0:
            print outandstat[1]
