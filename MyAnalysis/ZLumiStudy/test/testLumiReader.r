{ 
  ...
  TH1F* histDel = new TH1F("DelLumi", "; difference; Events", 100, -0.04, 0.04);
  TH1F* histDelLumi = new TH1F("DelLumiDiff", "; difference; Events", 100, -5, 5);
  TH1F* histRec = new TH1F("RecLumi", "; difference; Events", 100, -0.04, 0.04);
  TH1F* histRecLumi = new TH1F("RecLumiDiff", "; difference; Events", 100, -5, 5);
  TH2F* hist_ls_delLumi = new TH2F("ls_Lumi", "; ls; difference", 555, 0, 555, 100, -5, 5);
  TH2F* hist_ls_recLumi = new TH2F("ls_recLumi", "; ls; difference", 555, 0, 555, 100, -5, 5);
  ...
  RunLumiIndex endDel;
    ...
    for (int ls = 1; ls <= nLSs; ls++) {
      ...
      float sumDel_all = reader.getDelIntegral(startDel);
      float sumRec_all = reader.getRecIntegral(startDel);

      pair<float, float> totalLumi = reader.getTotalLumi(RunLumiIndex(run, ls));

      float diff_Del = (sumDel_all - totalLumi.first);
     // cout << sumDel_all << " : " << totalLumi.first << endl;
      float diff_Rec = (sumRec_all - totalLumi.second);
      ...
      //cout << diff_Del / totalLumi.first << " : " << diff_Del << " -- " << diff_Rec / totalLumi.second << " : " << diff_Rec << endl;
    }
